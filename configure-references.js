...
const config = JSON.parse(fs.readFileSync('tsconfig.base.json').toString());
config.files = [];
config.references = [];
...
(async function () {
  ...
  for (const name in workspaces) {
    ...
    if (fs.existsSync(tsconfigPath)) {
      ...
      for (const dependency of workspace.workspaceDependencies) {
        ...
        if (fs.existsSync(path.resolve(dependencyLocation, 'tsconfig.json'))) {
  ...
  fs.writeFileSync('tsconfig.base.json', JSON.stringify(config, undefined, 4));
})();
